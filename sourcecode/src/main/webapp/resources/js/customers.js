...
$(document).ready(function()
{
    ...
    //
    $("#edit-customers").on('show.bs.modal', function (event, m) {
        $(this).find('[name=id]').val(event.relatedTarget.dataset.id);
        $(this).find('[name=name]').val(event.relatedTarget.dataset.name);
        $(this).find('[name=address]').val(event.relatedTarget.dataset.address);
        $(this).find('[name=buildingName]').val(event.relatedTarget.dataset.buildingName);
        $(this).find('[name=longtitude]').val(event.relatedTarget.dataset.longtitude);
        $(this).find('[name=latitude]').val(event.relatedTarget.dataset.latitude);
        $(this).find('[name=note]').val(event.relatedTarget.dataset.note);
        $(this).find('[name=createBy]').val(event.relatedTarget.dataset.createBy);
    ...
    //
    var validEditAddress = true;

    $('.edit input[name*=address]').on('input', function () {
        var address = $(this).val();
        if (address.length < 1 || address.length > 255) {
            $('.vld-address').text("1-10文字　1byte文字　可変長!");
            $(this).closest('div.form-group').addClass('has-error');
            validEditAddress = false;
            return;
        }

        $('.vld-address').text("");
        $(this).closest('div.form-group').removeClass('has-error');
        validEditAddress = true;
    });
