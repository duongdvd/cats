...
<c:choose>
	<c:when test="${isCustomer}">
		<c:set var="titleCode" value="customer.edit.title"></c:set>
	</c:when>
	<c:otherwise>
		<c:set var="titleCode" value="garage.edit.title"></c:set>
	</c:otherwise>
</c:choose>
<%@include file="/resources/layout/pageHeader.jsp" %>
	...
	<div class="content-wrapper">
		...
		<section id="content" class="content">
			...
			<s:form method="POST" action="${ctx}/editCustomer" enctype="multipart/form-data" modelAttribute="customerForm" class="form-horizontal">
				<input type="text" hidden name="id" value="${customerForm.id}"/>
					...
					<div class="box-body">
						<div class="col-sm-6">
							<div class="form-group">
								<label for="divisionName" class="col-sm-4 control-label">
									<spring:message code="customer.title.divisionName"/></label>
								<div class="col-sm-8">
									<input type="hidden" name="id" value="${customerForm.id}" />
									<input type="hidden" name="divisionsId" value="${division.id}" />
									<input type="text" id="divisionName" name="divisionName" value="<c:out value="${division.divisionName}"/>" class="form-control" readonly/>
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input type="text" class="form-control" id="latitude" name="latitude" value="<c:out value="${customerForm.latitude}"/>"
				...
				<div class="text-right">
                    <button class="btn btn-primary btn-submit" type="submit" <c:if test="${editPermission eq false}"> disabled</c:if>><spring:message code="customer.btn.save"/></button>
					<c:choose>
						<c:when test="${isCustomer}">
